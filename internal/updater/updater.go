...
func (u *Updater) GetData(macAddress string) (govee.Data, error) {
	u.dataLock.RLock()
	defer u.dataLock.RUnlock()

	d, ok := u.dataMap[macAddress]
	if !ok {
		return govee.Data{}, fmt.Errorf("no sensor with MAC address registered: %s", macAddress)
	}

	if d.Data == nil {
		return govee.Data{}, errors.New("no data available")
	}
...
func (u *Updater) updateSensor(ctx context.Context, sensor config.Sensor) error {
	...
	data, err := govee.ReadData(ctx, u.log, u.device, sensor.MacAddress)
	if err != nil {
